...
export const Modal = ({ onClose, children, userParams }) => {
  ...
  const handleBackDropClick = e => {
    if (e.currentTarget === e.target) onClose(false);
  };
  ...
  return createPortal(
    <Overlay onClick={handleBackDropClick}>
      <ModalWindow
        onClose={onClose}
        style={location.pathname === routes.home && isMobile ? { top: '460px' } : null}
      >
        {backResponse ? (
          <DailyCalorieIntake backResponse={backResponse} userParams={userParams} />
        ) : (
          <Loader />
        )}
